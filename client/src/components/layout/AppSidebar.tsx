...
export function AppSidebar({ isCollapsed, onToggleCollapse }: AppSidebarProps) {
  ...
  return (
    <aside
      className={cn(
        "relative z-30 hidden h-screen shrink-0 flex-col justify-between py-4 bg-[var(--bg-raised)] border-r border-border lg:flex sidebar-transition",
    ...
    >
      ...
      <div className="flex flex-col min-w-0">
        ...
        {/* Navigation */}
        <nav className={cn("space-y-0.5", !isCollapsed && "px-1")}>
          {navigation.map((item) => {
      ...
      <div className="mt-auto flex flex-col min-w-0">
        {/* Facility block */}
        {!isCollapsed ? (
          <div className="rounded-xl border border-border bg-surface p-3 shadow-[var(--shadow-xs)]">
        ...
        <Link
          ...
          className={cn(
            ...
            isCollapsed ? "justify-center mx-auto h-10 w-10" : "gap-2.5 px-2.5 py-2 mx-1"
          )}
        >
